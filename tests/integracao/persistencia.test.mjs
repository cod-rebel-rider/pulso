...
test('aplicação inicia sem banco existente → banco criado com schema atual', () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(estado.versaoSchema, 5, 'o schema oficial (infraestrutura + jogador + status + missões + progressão) é aplicado');
      assert.deepEqual(
...
test('persistência completa: salvar → fechar → reabrir → ler (banco reutilizado)', () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(segundaExecucao.versaoSchema, 5, 'nenhuma migração deve rodar de novo');
