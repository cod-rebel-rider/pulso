...
test('aplicação inicia sem banco existente → banco criado com schema atual', () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(estado.versaoSchema, 13, 'o schema oficial (infraestrutura + jogador + status + missões + progressão + projetos + finanças + lista de desejos + conciliação + serviços + contas + recorrências + vínculo da geração) é aplicado');
      assert.deepEqual(
...
test('persistência completa: salvar → fechar → reabrir → ler (banco reutilizado)', () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(segundaExecucao.versaoSchema, 13, 'nenhuma migração deve rodar de novo');
