...
**Implementados:** Jogador (Fase 03) · Status (Fase 04) · Missões (Fase 05) · Progressão (Fase 06) · Projetos (Fase 07) · **Finanças (Fase 08)** · **Loja / Lista de Desejos (Fase 09)** · **Serviços (Fase 10.1)** · **Contas / Despesas (Fase 10.2)** — cada um com domínio próprio, casos de uso próprios, contrato IPC documentado e testes próprios. A Loja não tem carteira própria: toda movimentação financeira da compra passa pelo serviço da Fase 08 (ver ADR-011). As Contas não têm efeito financeiro algum: criar/editar/cancelar nunca cria transação nem altera saldo (ver `docs/contas-despesas.md`).

**Planejados (nenhum implementado):** `Recorrências (10.3) · Habilidades · Música · Mapa · Conquistas`
